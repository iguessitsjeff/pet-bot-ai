...
from src.config.loader import get_config_key, load_secrets
...
bot = telebot.TeleBot(token=get_config_key("TELEGRAM_TOKEN"))
...
def telegram():
    ...
    text_back: str = ""
    for message in lex_response.unpacked_messages:
        text_back = f"{text_back}{message.content}\n"

    bot.send_message(chat_id=tel_event.message.chat.id, text=text_back)

    if (
        lex_response.unpacked_session_state.intent.state
        is LexStates.READY_FOR_FULLFILLMENT
    ):
        prompt: str = f"{lex_response.unpacked_session_state.intent.slots.image_descriptor.value.interpreted_value} {lex_response.unpacked_session_state.intent.slots.animal.value.interpreted_value}"
